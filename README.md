...
- **Element collection** - sample state and container, purity, weight, isotope, origin (direct source / decay product with a multi-step decay chain), acquisition date and a full replacement history (previous samples, why each was replaced, whether it's still kept), photos, an interactive gamma spectrum - click to enlarge, isotope reference lines confirmed against the actual measurement (not just copied from a table), original XML available for download
- **Collection overview** - a dedicated panel with collection stats by category, every recorded gamma spectrum, elements available to buy, and a single date-sorted timeline of every acquisition/replacement across the whole collection
- **Radioactivity** - radioactive elements flagged on the table, NFPA 704 and GHS pictogram cards, isotope and half-life data
- **Production geography** - an interactive map on every element's card, all 118, including synthetic ones (highlighting the discovery/production facility's country) and industrial gases (highlighting the leading producing countries); where a real per-country breakdown is published, each country's opacity scales with its actual share of world production, with an on-hover percentage tooltip
- **Share** - share or copy a direct link to any element card straight from its header (Web Share API with a clipboard fallback)
...
| Production geography | own section on every one of the 118 elements - map of producing countries plus a short note |
...
- `physical.manufactureDate` is when the sample itself was made/produced, not when it joined your collection (see `acquiredDate` for that) - variable precision, whatever you actually know: `"2021"`, `"2021-07"`, or `"2021-07-31"`. If only a range is known, use `{ from, to }` with two such strings instead, e.g. `{ from: '1950', to: '1960' }` shows as "1950 – 1960". Either side can be left out if only the other end is known - `{ from: '2002' }` shows as "2002 –" (still made as of now), `{ to: '1962' }` shows as "– 1962" (start unknown).
