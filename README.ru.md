...
- **Коллекция элементов** - состояние образца и контейнер, чистота, изотоп, происхождение (прямой источник / продукт распада с цепочкой из нескольких изотопов), фото, интерактивный гамма-спектр - увеличение по клику, метки изотопных линий, подтверждённые по самому измерению (а не просто взятые из справочника), скачивание исходного XML
- **Обзор коллекции** - отдельная панель со статистикой по категориям и всеми записанными спектрами в одном месте
...
<details>
<summary><strong>Полный список свойств в карточке элемента</strong> (кликните, чтобы развернуть)</summary>
...
**Распространённость** - доля по массе во Вселенной, на Солнце, в океане, в организме человека, в земной коре и в метеоритах
...
- `myElements` - карта `символ → детали`. Чтобы отметить элемент своим, достаточно добавить ключ; одного `{}` уже хватает («он у меня есть, подробностей пока нет»). Поля `sampleState`, `container`, `purity`, `isotope`, `sourceType`, `decayParent`, `spectrum`, `spectrumAnnotations` заполняются постепенно и все необязательны.
- Если встроенного словаря `sampleState`/`container` (в [`src/locales/collection.ts`](src/locales/collection.ts)) не хватает - либо добавьте туда новую запись, либо вообще пропустите словарь и впишите готовый текст прямо в поле `description` элемента - пример такого подхода есть у радиоактивных элементов в `myCollection.ts`. `sourceType` - фиксированное значение `'primary'` / `'secondary'`.
...
Гамма-спектры (поля `spectrum`/`spectrumFilename`) - опциональны, указывайте их только если у вас правда есть файлы измерений для `src/data/spectra/`. Поле `spectrumAnnotations` отмечает на графике справочные гамма/рентгеновские линии (энергия в кэВ + подпись) - добавляйте линию только убедившись, что это и задокументированная линия излучения, и она реально видна на фоне именно в вашем измерении, а не просто взята из таблицы.
