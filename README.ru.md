...
- `physical.weight` - объект `{ mg, approx? }`: `mg` - вес в миллиграммах, `approx: true` отмечает его как оценочный, а не измеренный. На фронте показывается в мг, если меньше 1000, иначе переводится в граммы - `{ mg: 60, approx: true }` отобразится как «~60 мг», `{ mg: 1850, approx: true }` - как «~1.85 г».
...
- `physical.manufactureDate` - когда сам образец был изготовлен/произведён, а не когда попал в коллекцию (для этого есть `acquiredDate`) - точность произвольная, какая известна: `"2021"`, `"2021-07"` или `"2021-07-31"`. Если известен только интервал, вместо строки можно указать `{ from, to }` с такими же строками, например `{ from: '1950', to: '1960' }` отобразится как «1950 – 1960». Любую из сторон можно не указывать, если известна только другая - `{ from: '2002' }` отобразится как «2002 –» (выпускается до сих пор), `{ to: '1962' }` - как «– 1962» (начало неизвестно).
