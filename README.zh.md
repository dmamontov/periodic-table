...
- **元素收藏** - 样品状态与容器、纯度、重量、同位素、来源（直接来源 / 多步衰变链的衰变产物）、入藏日期以及完整的更换历史（历史样品、每次更换的原因、是否仍然保留）、照片，以及交互式伽马能谱——点击放大、经实测确认的同位素参考线（而非直接照搬表格数值）、可下载原始 XML
- **收藏总览** - 独立面板，按类别统计收藏进度，集中展示所有已记录的能谱、可购买的元素，以及整个收藏按日期排序的完整入藏/更换时间线
- **放射性标注** - 表格中标出放射性元素，提供 NFPA 704 与 GHS 象形图卡片、同位素与半衰期数据
- **产地** - 全部118个元素卡片上都有的交互式地图，包括合成元素（标示获得该元素的实验室/加速器所在国）和工业气体（标示主要生产国）；在有真实分国占比数据的地方，每个国家的透明度会反映其在世界产量中的实际份额，悬停可查看百分比提示
- **分享** - 直接从元素卡片头部分享或复制该元素的链接（使用 Web Share API，并支持复制到剪贴板作为后备方案）
...
- `physical.manufactureDate` 是样品本身的制造/生产日期，而不是加入收藏的日期（后者见 `acquiredDate`）- 精度不限，知道多少写多少：`"2021"`、`"2021-07"` 或 `"2021-07-31"`。如果只知道大致区间，可以用 `{ from, to }` 代替字符串，两侧各写一个这样的日期，例如 `{ from: '1950', to: '1960' }` 会显示为「1950 – 1960」。如果只知道其中一侧，另一侧可以省略 - `{ from: '2002' }` 会显示为「2002 –」（至今仍在生产），`{ to: '1962' }` 会显示为「– 1962」（起始日期未知）。
...
├── theme/             # 主题处理（浅色/深色/自动）与可配置的颜色体系（分类、热力图、收藏强调色、侧边栏分区）
...
元素参考属性、GHS 象形图、样品照片与 NFPA 704 评级，是从公开来源（PubChem、维基百科等）聚合而来的；开采/生产国数据来自 USGS Mineral Commodity Summaries 及其他公开来源；该功能使用的世界地图为 [`@svg-maps/world`](https://github.com/VictorCazanave/svg-maps)（CC-BY-4.0）；收藏的伽马能谱则为作者本人的实测数据。这些数据仅用于教育与个人项目用途。
