...
- 如果内置的 `sampleState`/`container`/`sourceType` 词汇表（[`src/locales/dictionaries.ts`](src/locales/dictionaries.ts) 中的 “Collection vocabulary” 部分）不够用，你可以在那里添加新条目，也可以完全跳过词汇表，直接把现成的文字写进该元素的 `description` 字段 —— `myCollection.ts` 里的放射性元素就是这么做的，可以参考。
