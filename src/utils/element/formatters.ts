...
export function formatIonChargeHtml(symbol: string, charge: string | null | undefined): string {
  if (!charge) return '----';
...
function nfpaWhiteDiamond(value: string): string {
  return nfpaWhiteDiamondParts(value)
    .map((part) => part.value)
    .join('');
}
...
function nfpaLabel(locale: Locale, category: 'nfpa1' | 'nfpa2' | 'nfpa3' | 'nfpa4', index: number | string): string {
  const labels = localeMessages[locale].nfpa;
  const table: Record<string, string> = labels[category];
  return table[`item${index}`] ?? '----';
}

export function buildNfpaDisplay(raw: string | null | undefined, locale: Locale): NfpaDisplay | null {
  if (!raw) return null;
  ...
  const score = (value: string, num: number) => (value === '-' || Number.isNaN(num) ? '' : String(num));
